...
export const AddNote = ({
  ...
  const length = (note ? note.length + 1 : 0) + (comentarios ? comentarios.length : 0);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      triggerRefetch();
      toast({ description: 'Nota agregada correctamente' });
    } catch (error) {
      toast({ variant: 'destructive', title: 'Error', description: (error as any).message });
    } finally {
      closeMenu();
    }
  };
  ...
  if (length >= MAX_LENGTH - 10) return;
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              layoutId={`popover-${uniqueId}`}
              className="absolute -right-[85px] -top-2 h-[200px] w-[calc(100vw-100px)] max-w-[400px] overflow-hidden border border-zinc-950/10 bg-white outline-none dark:bg-zinc-700 sm:w-[364px] md:w-[calc(40vw)]"
              style={{
                borderRadius: 12,
              }}
            >
              ...
              >
                ...
                <textarea
                  ...
                  maxLength={MAX_LENGTH - (comentarios ? comentarios.length : 0)}
                />
                <p className="flex justify-end pr-4 pt-3 text-xs text-muted-foreground">
                  {length} / {MAX_LENGTH}
                </p>
