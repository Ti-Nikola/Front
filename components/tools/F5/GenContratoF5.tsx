...
export const DocumentGenerationCard: React.FC<DocumentGenerationCardProps> = ({
  ...
  const [representativeRUT, setRepresentativeRUT] = React.useState(project.rut_facturacion || '');
  const [useDefaultRepresentativeRUT, setUseDefaultRepresentativeRUT] = React.useState(true);
  ...
  const handleGenerate = () => {
    ...
    const finalRepresentativeRUT = useDefaultRepresentativeRUT
      ? project.rut_facturacion
      : representativeRUT;
    ...
    if (distribuidora === 'Enel' && personalidadJuridica === 'natural') {
      ...
      docJuridica({
        ...
        clientRUT: finalRUT,
        clientName: finalClientName,
        representativeRUT: finalRepresentativeRUT,
        distribuidora,
        personalidadJuridica,
      });
  ...
  return (
    <div className="justify-left mt-6 flex flex-col">
      ...
      {personalidadJuridica === 'juridica' && (
        <div className="mb-4">
          <label>RUT del representante legal:</label>
          <input
            type="text"
            value={representativeRUT}
            onChange={(e) => setRepresentativeRUT(e.target.value)}
            className="rounded border px-2 py-1"
            disabled={useDefaultRepresentativeRUT}
          />
          <div>
            <input
              type="checkbox"
              checked={useDefaultRepresentativeRUT}
              onChange={() => setUseDefaultRepresentativeRUT(!useDefaultRepresentativeRUT)}
            />
            <label>Usar RUT del representante por defecto</label>
          </div>
        </div>
      )}
      <Button
